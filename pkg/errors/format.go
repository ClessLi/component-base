...
func format(k int, jsonData []map[string]interface{}, str *bytes.Buffer, finfo *formatInfo,
	...
	if modeJSON {
		...
		if flagDetail || flagTrace {
			if finfo.stack != nil {
				f := Frame((*finfo.stack)[0])
				fmt.Fprintf(str, "%s%s - #%d [%s:%d (%s)] (%s:%d) %s",
					sep,
					finfo.err,
					k,
					f.file(),
				...
				fmt.Fprintf(str, "%s%s - #%d (%s:%d) %s", sep, finfo.err, k, finfo.namespace, finfo.code, finfo.message)
			}
...
func buildFormatInfo(e error) *formatInfo {
	...
	case *withCode:
		coder := findCoderByCodeInNamespace(err.code, err.namespace)
		if coder == nil {
			coder = unknownCoder
		}
